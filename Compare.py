...
class Example(Frame):
    ...
    def initUI(self): 
        ...
        self.sb2.grid(row = 1, column = 2, rowspan = self.textRow, padx=0, sticky = E+S+N)
        self.text2['yscrollcommand'] = self.sb2.set 
        ...
        abtn = Button(self, text="Open1", command=self.onOpen)
        abtn.grid(row=12, column=self.textColumn +3, pady=1, sticky = W)
    ...
    def onOpen_C(self):
        ...
        if f3 != '':
            ...
            text = self.readFile_C(f3)
            self.text2.insert(END, text)
            self.setNameLabel("File: " + f3)
    ...
    def autoLoadNewFile_C(self, fileName, newcursor_index):
        """自动加载新文件"""
        if self.debug:
            print("Action Track: autoLoadNewFile")
        if len(fileName) > 0:
    ...
    def setColorDisplay_C(self):
        if self.debug:
            print("Action Track: setColorDisplay")
        self.text2.config(insertbackground='red', insertwidth=4, font=self.fnt)
        countVar = StringVar()
        ...
        if self.colorAllChunk:
            for flag in list('abcdefgh'):
                self.text2.mark_set("matchStart"+flag, "1.0")
                self.text2.mark_set("matchEnd"+flag, "1.0")
                self.text2.mark_set("searchLimit"+flag, 'end-1c')
            self.text2.mark_set("recommend_matchStart", "1.0")
            self.text2.mark_set("recommend_matchEnd", "1.0")
            self.text2.mark_set("recommend_searchLimit", 'end-1c')
        else:
            for flag in list('abcdefgh'):
                self.text2.mark_set("matchStart"+flag, lineStart)
                self.text2.mark_set("matchEnd"+flag, lineStart)
                self.text2.mark_set("searchLimit"+flag, lineEnd)
            self.text2.mark_set("recommend_matchStart", lineStart)
            self.text2.mark_set("recommend_matchEnd", lineStart)
            self.text2.mark_set("recommend_searchLimit", lineEnd)
        
        #为标签设置颜色
        for key in self.pressCommand:
            while True:
                pattern = r'\[\@.*?\#' + self.pressCommand[key] + r'\*\](?!\#)'
                self.text2.tag_configure("catagory"+key, background=self.labelEntryColor[key])
                pos = self.text2.search(pattern, "matchEnd"+key, "searchLimit"+key,  count=countVar, regexp=True)
                if pos =="":
                    break
                self.text2.mark_set("matchStart"+key, pos)
                self.text2.mark_set("matchEnd"+key, "%s+%sc" % (pos, countVar.get()))
        ...
        ## color recommend type
        while True:
            self.text2.tag_configure("recommend", background=self.recommendColor)
            recommend_pos = self.text2.search(self.recommendRe, "recommend_matchEnd" , "recommend_searchLimit",  count=countVar, regexp=True)
            if recommend_pos =="":
                break
            self.text2.mark_set("recommend_matchStart", recommend_pos)
            self.text2.mark_set("recommend_matchEnd", "%s+%sc" % (recommend_pos, countVar.get()))
            
            first_pos = recommend_pos
            # second_pos = "%s+%sc" % (recommend_pos, str(1))
            last_pos = "%s+%sc" % (recommend_pos, str(int(countVar.get())))
            self.text2.tag_add("recommend", first_pos, last_pos)            
        
        ## color the most inside span for nested span, scan from begin to end again  
        if self.colorAllChunk:
            self.text2.mark_set("matchStart", "1.0")
            self.text2.mark_set("matchEnd", "1.0")
            self.text2.mark_set("searchLimit", 'end-1c')
        else:
            self.text2.mark_set("matchStart", lineStart)
            self.text2.mark_set("matchEnd", lineStart)
            self.text2.mark_set("searchLimit", lineEnd)
        while True:
            self.text2.tag_configure("insideEntityColor", background=self.insideNestEntityColor)
            pos = self.text2.search(self.insideNestEntityRe , "matchEnd" , "searchLimit",  count=countVar, regexp=True)
            if pos == "":
                break
            self.text2.mark_set("matchStart", pos)
            self.text2.mark_set("matchEnd", "%s+%sc" % (pos, countVar.get()))
            first_pos = "%s + %sc" %(pos, 2)
            last_pos = "%s + %sc" %(pos, str(int(countVar.get())-1))
            self.text2.tag_add("insideEntityColor", first_pos, last_pos) 


    def compareTwoText(self):
        if self.debug:
            print("Action Track: setColorDisplay")
        self.text.config(insertbackground='Yellow', insertwidth=4, font=self.fnt)
        self.text2.config(insertbackground='Yellow', insertwidth=4, font=self.fnt)
        countVar = StringVar()
        countVar2 = StringVar()
        currentCursor = self.text.index(INSERT)
        currentCursor2 = self.text2.index(INSERT)
        lineStart = currentCursor.split('.')[0] + '.0'
        ...
        for key in self.pressCommand:
            while True:
                ...
                pos = self.text.search(pattern, "matchEnd"+key, "searchLimit"+key,  count=countVar, regexp=True)
                pos2 = self.text2.search(pattern, "matchEnd"+key, "searchLimit"+key,  count=countVar2, regexp=True)
                if pos =="" or pos2 == "":
                    break
                self.text.mark_set("matchStart"+key, pos)
                self.text.mark_set("matchEnd"+key, "%s+%sc" % (pos, countVar.get()))
                self.text2.mark_set("matchStart"+key, pos2)
                self.text2.mark_set("matchEnd"+key, "%s+%sc" % (pos2, countVar2.get()))
                    
                first_pos = pos
                last_pos = "%s + %sc" %(pos, str(int(countVar.get())))
                first_pos2 = pos2
                last_pos2 = "%s + %sc" %(pos2, str(int(countVar2.get())))
                ...
                for e in commandList:
                    ...
                    if containing != None and containing2 != None:
                        exist = True
                        break
                if exist:
                    self.text.mark_set("matchEnd"+key, "%s+%sc" % (pos, str(1)))
...
## save as new shortcut map
    ...
    def setMapShow(self):
        ...
        for key in sorted(self.pressCommand):
            ...
            symbolLabel.grid(row=row, column = 7, columnspan=1, rowspan = 1,padx= 1, sticky=W+E+S+N)
            ...
            labelEntry.grid(row=row, column = 9, columnspan=3, rowspan = 1,padx=2,sticky=W)
